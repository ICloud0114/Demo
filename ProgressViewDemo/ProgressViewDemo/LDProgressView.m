...
- (void)setAnimate:(NSNumber *)animate {
    ...
    if ([animate boolValue]) {
        self.timer = [NSTimer scheduledTimerWithTimeInterval:0.05 target:self selector:@selector(incrementOffset) userInfo:nil repeats:YES];
    } else if (self.timer) {
...
- (void)drawProgress:(CGContextRef)context withFrame:(CGRect)frame {
    ...
    if ([self.showStroke boolValue]) {
        CGContextSetStrokeColorWithColor(context, [self.color darkerColor].CGColor);
        [roundedRect stroke];
    }
