...
- (NSString *)prepareSql {
    NSString *sql = [NSString stringWithFormat:@"INSERT INTO %@ (uid, password, phone, email, nickName, headImage, address, sex, birthday) VALUES('%@','%@','%@','%@','%@','%@','%@',%d,'%@')", [[self class] tableName], self.uid, self.password, self.phone, self.email, self.nickName, self.headImage, self.address, self.sex, self.birthday];
    return sql;
}
