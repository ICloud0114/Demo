...
@property (nonatomic, strong) QueryDataCmd *readTableCmd;
...
- (void)saveTable:(NSString *)tableN data:(NSArray *)arr completion:(void (^)(void))handler {
    Class<TableCoding> tableClass = [[TBDatabaseManager shareInstance].tableModelMaps objectForKey:tableN];
    NSMutableArray<id<TableCoding>> *tableModels = @[].mutableCopy;
    for (NSDictionary *dic in arr) {
        //        [self updateReadTableTime:dic[@"updateTime"]];
        id<TableCoding> obj = [tableClass performSelector:@selector(mj_objectWithKeyValues:) withObject:dic];
        if (obj) {
    ...
    inTransaction(^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
        [tableModels enumerateObjectsUsingBlock:^(id<TableCoding>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)reloadLostPageData {
    ...
    [self cancelTimeoutWithReadTableInfo:tableInfo];
    [self addTimeoutWithReadTableInfo:tableInfo];
    QueryDataCmd *cmd = [self readLostTableCmd];
    if (cmd) {
        __weak typeof(self) weakSelf = self;
...
- (QueryDataCmd *)readTableCmd {
    if (!_readTableCmd) {
        _readTableCmd = [QueryDataCmd cmdInstance];
...
- (QueryDataCmd *)readLostTableCmd {
    NSArray *lostPages = [self lostPages];
    if (lostPages.count > 0) {
        QueryDataCmd *lostCmd = [QueryDataCmd cmdInstance];
